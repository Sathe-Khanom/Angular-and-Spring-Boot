...
export class JobSeekerProfileComponent implements OnInit {
  ...
  addSummery(): void {
    this.summeryService.addSummery(this.newSummery).subscribe({
      next: () => {
        this.newSummery = {
          fatherName: '',
          motherName: '',
      ...
      error: (err) => console.error('Failed to add summery', err)

    })
  }

    deleteSummery(id: number | undefined): void {
    if (!id) return;
    if (!confirm('Are you sure you want to delete this summery?')) return;

    this.summeryService.deleteSummery(id).subscribe({
      next: () => {
        this.loadSummery();
        this.cdr.markForCheck();
      },
      error: (err) => console.error('Failed to delete summery:', err)
    });
  }
